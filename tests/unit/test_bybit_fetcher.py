...
class TestBybitFetcherSynthetic:
    ...
    async def test_fetch_synthetic_kline_spans_rebrand(self, mock_fetch_kline):
        ...
        df_ton = pl.DataFrame({
            ...
            "open": [1.0], "high": [1.1], "low": [0.9], "close": [1.05], "volume": [100.0], "turnover": [105.0],
            "datetime": [datetime(2026, 6, 1, tzinfo=UTC)]
        })
        df_gram = pl.DataFrame({
            "timestamp": [1719000000000],
            "open": [1.05], "high": [1.2], "low": [1.0], "close": [1.15], "volume": [200.0], "turnover": [230.0],
        ...
        result = await fetcher.fetch_synthetic_kline(symbol="GRAMUSDT", start_time=start, end_time=end)
